...
//		for(Task task : tasks) {
//			if(task.pickupCity == currentState.getCity() && !currentState.getDelivered().contains(task) && !currentState.getCarried().contains(task)) availableTasks.add(task);
...
//
//
//			TaskSet delivered = TaskSet.intersect(currentState.getCarried(), tasksToBeDelivered);
//			TaskSet carried = TaskSet.intersectComplement( currentState.getCarried(), delivered);
//			TaskSet allDelivered = TaskSet.union(currentState.getDelivered(), delivered);
//
...
//				TaskSet alreadyDeliveredSubset = TaskSet.noneOf(tasks);
//
//				for (Task task : tasksSubset) {
//					availableTasksSubset.add(task);
//				}
//
//				TaskSet allCarried = TaskSet.union(carried, availableTasksSubset);
//				if(allCarried.weightSum() <= vehicle.capacity()) {
//					TaskSet allDelivered2 = TaskSet.copyOf(allDelivered);
//					TaskSet allCarried2 = TaskSet.copyOf(allCarried); // obligé? (on parcoure en meme temps qu'on remove)
//					for (Task task : allCarried) {
//						if(task.deliveryCity==neighbor) {
//							allCarried2.remove(task);
//							allDelivered2.add(task);
//						}
//					}
//
//					State state = new State(neighbor, allCarried2, allDelivered2, tasks);
//
...
//					}
//
//					State state = new State(neighbor, allCarried2, allDelivered2, tasks, allCarried2.weightSum());
//
//					Node node = new Node(state, currentNode, cost);
//					successors.add(node);
//				}
...
//	}
	...
	public boolean isFinal(State s, TaskSet tasks, Vehicle v){
		...
		//return carried U delivered == vehicle.getCurrentTasks U tasks
