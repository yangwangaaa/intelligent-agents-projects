...
public class AgentMeanOptimistic implements AuctionBehavior {
	...
	private void computeRealCost(ArrayList<Double>mc1,ArrayList<Double>[]mc2,Task task) {
		...
		int NtoAdd = (expectedNumberOfTasks-tasks1.size());  //TODO
		...
		for(int j=0; j<Nconf; j++) {
			...
			NtoAdd = (expectedNumberOfTasks-tasks2.size());
			...
			print("proposed = "+proposed+" expected Number of task = "+ expectedNumberOfTasks +"size tasks1 = "+tasks1.size()+" size tasks2 = "+tasks2.size());
			print("size task clone 1 = "+ tasks1Clonea.size());
			print("size task clone 2 = "+ tasks2Clonea.size());
		}
	}
