...
public class BFS {
	...
	public BFS(Topology topology, TaskSet tasks,  Vehicle vehicle, int[] start,  int[] goal){
	...
	//=============use BFS algorithm to get shortest path===========
	public List<TreeNode> getShortestPath(){
		...
		while(!openSet.isEmpty()){
			TreeNode current = openSet.poll();
			closedSet.add(current);
			
			if(goal_reached(current, goal) == true){
				if(current.g_score < min_cost){
					min_cost =  current.g_score;
					res_index = current.index;
					continue;
				}
				else{
					continue;
			...
			for(TreeNode neighbor: neighbors){
				...
				if(res_index != -1 && tentative_g_score >= min_cost) 
					continue;
		...
		if(res_index != -1){
			...
			return reconstruct_path(all_states.get(res_index));
		}
		else
			return null;
	}
