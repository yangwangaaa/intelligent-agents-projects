...
public class AgentMean implements AuctionBehavior {
	...
	public List<Plan> plan(List<Vehicle> vcls, TaskSet tasksSet) {
		...
		print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
		print("AGENT MEAN : number of tasks = " + tasksSet.size());
	...
	public Long askPrice(Task task) {
		...
		print("----- AGENT MEAN ASK PRICE: T"+ proposed+ "=" + task +" id = "+agent.id()+" -----");
		...
		print("----- AGENTMEAN : BIDDING TASK " + task.id + ", Bid = " + Math.round(b) + ", in " + duration + " sec");
		print("");
	...
	private void computeRealCost(ArrayList<Double>mc1,ArrayList<Double>[]mc2,Task task) {
		...
		if(NtoAdd>=1) {
			...
			//real cost
			double realCost1 = Math.max(cost2/(double)tasks1Cloneb.size() + (cost2*(tasks1.size()/(double)tasks1Cloneb.size()) - cost1*(tasks1.size()/(double)tasks1Clonea.size())), 0);
			mc1.add(realCost1);
		...
		for(int j=0; j<Nconf; j++) {
			...
			if(NtoAdd>=1) {
				...
				double realCost2 = Math.max(cost2/(double)tasks2Cloneb.size() + (cost2*(tasks1.size()/(double)tasks2Cloneb.size()) - cost1*(tasks1.size()/(double)tasks2Clonea.size())), 0);
				mc2[j].add(realCost2);
				print("mean new cost = "+cost2/(double)tasks2Cloneb.size()+" money to pay more = "+(cost2*(tasks1.size()/(double)tasks2Cloneb.size()) - cost1*(tasks1.size()/(double)tasks2Clonea.size())));
	...
	public void auctionResult(Task previous, int winner, Long[] bids) {
		print("----- AuctionResult Agent Mean: T"+ proposed +", agent is "+agent.id()+" -----");
	...
	public void printInfoAgent() {
		print("------------------------------------------------------------------------------");
		print("AGENTMEAN " + agent.id() + " INFORMATIONS :");
